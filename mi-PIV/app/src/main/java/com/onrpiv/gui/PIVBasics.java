...
public class PIVBasics extends LearnPIV {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String html1 = "<b>Figure 1.</b> Example laboratory PIV set-up, where a camera images a laser light sheet that is illuminating a flow field seeded with particles.<br><br>As shown in the figure above, laser sheet optics (i.e., a cylindrical lens) focus the collimated laser beam into a thin laser sheet. The laser sheet illuminates neutrally buoyant particles(i.e., seeds) that have been introduced into the flow. These small(1-100 \uD835\uDF07m) seeds both follow the flow field and effectively scatter the laser light so that they can be seen and imaged. A digital camera (directed orthogonal to the laser sheet plane)images the particles in the plane as shown in below. The velocity can be calculated by taking the displacement of the particles divided by a known time step. In general, the particles shouldn’t move too much, the flow field being images should be the same between time steps and the time should be small enough that the particles have only moved a few particle diameters.";
